...
export async function deleteTheme(id: string): Promise<void> {
  ...
  if (s.defaultThemeId === id) {
    await api.updateSettings({ ...s, defaultThemeId: "seriesly" });
    settings.set({ ...s, defaultThemeId: "seriesly" });
  }
  await loadThemes();
}
...
export function effectiveTheme(profileThemeID: string): Theme | undefined {
  ...
  const s = get(settings);
  const id = profileThemeID || s.defaultThemeId || "seriesly";
  return all.find((t) => t.id === id) ?? all.find((t) => t.id === "seriesly") ?? all[0];
}
