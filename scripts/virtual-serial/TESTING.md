...
Every file-transfer test below uses a payload from `test/transfers/`
— deterministic binary blobs generated from a fixed RNG seed so two
runs of the same test compare against the same source bytes. Generate
them once at the repo root:
...
```
      2048  hex-1k.txt        — paste into Send Hex…       (T5)
   1048576  payload-1m.bin    — 1 MiB YMODEM source        (T9, T10)
      4096  payload-4k.bin    — 4 KiB YMODEM / XMODEM      (T6, T7)
       512  payload-512.bin   — slow-link YMODEM source    (T11)
         6  tiny.txt          — "hello\n" single-block     (T8)
```

The directory is gitignored. Rerun the script any time the files are
missing — output is bit-for-bit identical across runs (seeded RNG).
...
```sh
# Copy the prebuilt deterministic hex string to clipboard
pbcopy < test/transfers/hex-1k.txt              # macOS
# or: xclip -selection clipboard < test/transfers/hex-1k.txt   # Linux
```
...
**Source:** `test/transfers/payload-4k.bin` (regenerated by
`scripts/transfer-tests/regen-fixtures.sh`).
...
`rb` will print `Receiving: payload-4k.bin` once Baudrun sends the
...
1. ⋯ → **Send File…**
2. **Protocol:** YMODEM
3. **File:** `test/transfers/payload-4k.bin`
4. **Send**.

**Expected:**

- Progress bar fills 0 → 4096 over ~0.5 s at 115200 baud.
- Baudrun status: `Sent payload-4k.bin`.
- `rb` exits cleanly.

**Verify:**

```sh
diff test/transfers/payload-4k.bin /tmp/baudrun-rx/payload-4k.bin && echo MATCH
```
...
```sh
cmp -n $(wc -c < test/transfers/payload-4k.bin) \
    test/transfers/payload-4k.bin /tmp/baudrun-rx/out.bin && echo MATCH
```
...
**Source:** `test/transfers/tiny.txt` (6 bytes: `hello\n`).
...
cmp -n 6 test/transfers/tiny.txt /tmp/baudrun-rx/out.bin && echo HEAD-MATCH
xxd /tmp/baudrun-rx/out.bin | tail -3                      # trailing 1a 1a 1a ...
```

### T9 — large file, many blocks

**Source:** `test/transfers/payload-1m.bin` (exactly 1 MiB).
...
```sh
diff test/transfers/payload-1m.bin /tmp/baudrun-rx/payload-1m.bin && echo MATCH
```
...
Restart the bridge at 9600 baud and repeat T6 with `test/transfers/payload-512.bin`
(512 bytes). Exercises:
